...
impl TicketValidation {
    ...
    pub async fn validate_post_note(
        input: &handlers::ticket::NoteInput,
        session: &model::session::Session,
        db: &FirestoreDb,
    ) -> Result<(
        Option<TicketValidation>,
    ...
    pub async fn validate_post_comment(
        ...
        let mut comment: Option<model::comment::Comment> = None;
        if action == crate::Action::Update {
            comment = match model::comment::Comment::find(&input.comment_id, &db).await {
                Ok(t) => t,
                Err(e) => {
                    return Err(anyhow::anyhow!(e));
                }
            ...
            if let Some(c) = &comment {
                if c.uid != session.uid {
                    return Err(anyhow::anyhow!(
                        "他のユーザーのコメントは更新できません。".to_string()
                    ));
                }
            } else {
                return Err(anyhow::anyhow!("コメントが存在しません。".to_string()));
            }
        }

        Ok((None, project, project_member, ticket, comment))
    }
}
